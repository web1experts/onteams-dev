    ...
    export default (state = initialState, action) => {
      switch (action.type) {
            ...
            const updatedTasks = (state.tasks.taskData[tabId]?.tasks || []).map((task) => {
                if (task._id === taskId) {
                    ...
                    if (existingComment) {
                        ...
                        console.log('new comment', [action.payload.comment, ...task.comments]);
                        return {
                            ...task,
        ...
        case DELETE_COMMENT: {
            ...
            // If `tasks` is organized by tab
            const updatedTasks = { ...state.tasks?.taskData };

            // Loop through each tab's tasks to find and update the appropriate task
            // Object.keys(updatedTasks).forEach((tabId) => {
                // Map through the tasks under this tab
                if( updatedTasks[tab].tasks ){
                    updatedTasks[tab].tasks = updatedTasks[tab]?.tasks.map((task) => {
                        if (task._id.toString() === feedId.toString()) {
                            ...
                            const updatedComments = task.comments.filter((comment) => comment._id !== commentId);
            ...
            // Return the updated state with modified tasks
            return {
                ...state,
                tasks: updatedTasks,
            };
