...
import {
  ...
  deleteHoliday,
  updateHoliday,
} from "../../redux/actions/holiday.action";
...
function HolidaysPage() {
  ...
  const getStatusBadge = (dateString) => {
    ...
    if (inputDate < today)
      return (
        <Badge className="mt-1 mt-xl-0" bg="secondary">
          past
        </Badge>
      );
    return (
      <Badge className="mt-1 mt-xl-0" bg="success">
        upcoming
      </Badge>
    );
  };
              ...
              <Col md={12}>
                <h2>
                  ...
                  <ListGroup horizontal className="ms-auto">
                        ...
                        <Dropdown.Menu>
                          <div className="drop--scroll">
                            {years.map((year) => (
                              ...
                              >
                                {year}{" "}
                                {year === filters["year"] && <MdOutlineCheck />}
                              </a>
                            ))}
                          </div>
                        </Dropdown.Menu>
                    ...
                    <ListGroup
                      horizontal
                      className="bg-white expand--icon d-md-flex"
                    >
                      <ListGroup.Item
          ...
          {spinner ? (
            ...
            <Container fluid>
              ...
              <div
                className={
                  isActiveView === 1
                    ? "project--grid--table holiday--table--grid"
                    : isActiveView === 2
                    ? "project--table holiday--table holidays--list bg-white"
                    : "project--table holiday--table holidays--list bg-white"
                }
              >
                <h3 className="mb-4 d-flex align-items-center gap-3">
                  <span>
                    <FiCalendar />
                  </span>
                  Holiday Calendar - {filters["year"]}
                </h3>
                <Table>
                  <tbody>
                      ...
                      holidays.map((holiday, index) => {
                        ...
                        return (
                          <tr
                            ...
                            className={isUpcoming ? "upcoming-holiday" : ""}
                          >
                            <td key={`date-td-${index}`} data-label="Date">
                              <div className="project--name d-flex align-items-center gap-2 gap-md-4">
                                ...
                                <div className="title--span d-flex align-items-start gap-1 flex-column">
                                  <h5
                                    className="d-xl-flex gap-3 align-items-start"
                                    key={`occasion-td-${index}`}
                                    data-label="Occasion"
                                  >
                                    {holiday.occasion}{" "}
            ...
            <Form.Group className="form-group">
              <DatePicker
                ...
                onChange={async (value) => {
                  ...
                  const formattedDate =
                    selectedDate.toLocaleDateString("en-CA"); // 'en-CA' gives "YYYY-MM-DD" format
                  handleChange({
                ...
                <Dropdown.Menu>
                  <div className="drop--scroll">
                    ...
                    <Dropdown.Item
                      onClick={() => {
                        handleChange({
                          target: { name: "type", value: "full day" },
                        });
                      }}
                    >
                      Full Day{" "}
                      {fields["type"] === "full day" ? <FaCheck /> : null}
                    </Dropdown.Item>
                    <Dropdown.Item
                      onClick={() => {
                        handleChange({
                          target: { name: "type", value: "half day" },
                        });
                      }}
                    >
                      Half Day
                      {fields["type"] === "half day" ? <FaCheck /> : null}
                    </Dropdown.Item>
                  </div>
                </Dropdown.Menu>
              </Dropdown>
